...
def train(rank, world_size):
    ...
    dist.init_process_group(backend = 'nccl', init_method = 'tcp://localhost:57890', rank = rank, world_size = world_size)
    
    model = AgeModel().to(rank)
    ...
    train_sampler = DistributedSampler(train_age_data, num_replicas = world_size, rank = rank)
    train_age_loader = DataLoader(train_age_data, batch_size = batch_size, sampler = train_sampler,
                                     shuffle = False, num_workers = 0, pin_memory = True)
    
    test_age_loader = DataLoader(test_age_data, batch_size = batch_size,
                                     shuffle = False, num_workers = 0, pin_memory = True)
    ...
    for epoch in range(epochs):
        ...
        # accuracy
        train_accuracy = model_accuracy(train_age_loader, model, rank)
        valid_accuracy = model_accuracy(test_age_loader, model, rank)
        print("Epoch [{}/{}], Rank {}, Loss: {:.3f}, Training Accuracy: {:.2f}, Validation Accuracy: {:.2f}".format(epoch + 1, epochs, rank, total_loss/len(train_age_loader), train_accuracy, valid_accuracy))
    ...
    torch.save(model.state_dict(), "{}/age_model_ddp.pt".format(folder))
